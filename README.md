...
**Реализован этап 8 из 15: экспериментальный генератор платья и сарафана с деталями, парами швов, маркировкой и SVG-предпросмотром. Автоматический gate пройден; проверка закройщиком и макетом ожидается.**
...
Это ещё не готовое лекало для раскроя. Сборка и SVG работают, но припуски, линии среза и печать 1:1 относятся к этапу 9. Полные бумажные построения и экспертная проверка отсутствуют. **Исходный gate этапа 2 остаётся BLOCKED.** Успешные численные тесты не означают проверенную посадку.
...
Проверяются этапы 2–8: контракты, backend, мастер мерок, 36 тестов геометрического ядра, базовые блоки, 60 новых property-based примеров, UI production-сборка и сквозные smoke-тесты. API-ключи и Docker для проверки не нужны.
