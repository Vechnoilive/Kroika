...
Это реализованный на этапе 11 сценарий приложения: проект связывает изображения, подписанный Qwen/mock-анализ, ручное подтверждение фасона, мерки, ткань, прибавки, построение, проверку и экспорт. Каждый завершённый шаг создаёт локальную ревизию.

## Основной путь

| Шаг / экран | Действие закройщика | Ответ системы | Условие перехода |
| --- | --- | --- | --- |
| 1. Проект | Создаёт проект, затем выбирает один из десяти ограниченных типов | Черновик, краткие подсказки, точный перечень доступных компонентов | Выбран поддержанный тип |
...
| 9. Бумага и макет | Проверяет печать на двух принтерах, получает заключение конструктора и шьёт три макета на разных фигурах | Append-only журнал выводит статус из записей именно этой генерации | Допуск открыт только после всех записей; новая генерация начинает журнал заново |
...
| Квадрат после печати имеет неверный размер | Запись получает `failed`; пользователь исправляет масштаб и добавляет повторную запись, не удаляя прежнее наблюдение |
...
Автоматический путь повторяется в mock. Для закрытия внешней части gate тот же Playwright-сценарий должен быть пройден с реальным Qwen при серверных `QWEN_API_KEY` и `QWEN_BASE_URL`. Ни рисунок фасона, ни успешный ответ модели не подменяют построение лекал.
