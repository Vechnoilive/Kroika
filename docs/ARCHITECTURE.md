...
Версия контрактов: **1.0.0** · Этап расширения: **16/20** · Стиль: модульный монолит.
...
`supported`, `planned` или `needs_confirmation` и ссылку на разрешённый модуль. Только
полностью поддержанный план может стать подтверждённым `GarmentSpec` и попасть в
...
| `GarmentDesignIntent` | Прослеживаемый план деталей, слоёв и пропорций с фото | Неподдержанный/сомнительный элемент блокирует подтверждение и генерацию |
