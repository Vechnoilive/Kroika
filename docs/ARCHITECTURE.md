...
Версия контрактов: **1.0.0** · Этап: **7/15** · Стиль: модульный монолит.
...
Вспомогательные схемы определяют AI-запрос/ответ, запрос/ответ Pattern Engine и каноническую геометрию `PatternData`. Геометрическое ядро этапа 6 реализует линии, кубические Безье, эллиптические дуги и контуры. Этап 7 добавляет отдельный слой F01–F41 и экспериментальные блоки лифа, юбки и рукава.
