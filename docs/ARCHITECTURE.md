...
Версия контрактов: **1.0.0** · Этап расширения: **22/22** · Стиль: модульный монолит.
...
проверяются парные срезы. Исторические формулы кокетки, панелей и переноса вытачки
получили ограниченные исполняемые модули на этапе 21.

Компилятор этапа 21 выполняется после модельных операций и до составных деталей.
Прямая парная кокетка делит перед и спинку юбки, поглощает талиевые вытачки и создаёт
явные швы соединения. Панельный модуль заменяет половины юбки на 2–6 равных панелей,
разрезает сопрягаемый талиевый срез и создаёт вертикальные пары швов. Ограниченный
перенос передней вытачки сокращает талиевый раствор, увеличивает боковой и изменяет
контур так, чтобы закрытые длины не изменились. Операции аудируются в
`topology_operations`; несовместимые юбочные модули блокируются до генерации.

Компилятор этапа 19 выполняется после модельных преобразований и до припусков. Он
создаёт отдельные детали манжеты, стойки, накладного кармана, юбочной подкладки и
верхнего юбочного слоя. Бинарные соединения добавляются в `seam_pairs`, а настрочное
место кармана — замкнутым внутренним путём целевой детали. Все действия аудируются в
`composite_operations`; неизвестный или конфликтующий модуль остаётся fail-closed.

Компилятор этапа 20 выполняется после составных преобразований и до припусков. Он
индексирует доказательства доступных модулей в `design_coverage`: детали, пары швов,
пути, сегменты и операции. Активный подтверждённый модуль без доказательства
отклоняет результат. Frontend затем сопоставляет этот каталог с актуальными
идентификаторами фото и ручными решениями, поэтому исключение не исчезает, а пропуск
между фото и планом остаётся видимым. Hash-контракт `1.3.0` включает набор
обязательных модулей покрытия; наличие топологической операции поднимает его до
`1.4.0` и не переиспользует кеш другой геометрии.
...
| `PatternData.design_coverage` | Каталог доказательств модулей в готовой геометрии | Подтверждённый модуль без детали, линии, соединения или операции блокирует результат |
| `ValidationReport` | Машиночитаемые проверки и понятные сообщения | Блокирующая ошибка исключает производственный экспорт |
| `PhysicalValidationRecord` | Неизменяемое наблюдение печати, конструктора или макета для точной генерации | Допуск выводится из последних записей двух принтеров, эксперта и трёх разных профилей; запись другой генерации не учитывается |
...
`input_hash` вычисляется из значений, способных изменить геометрию или область применимости. Имена, время подтверждения и происхождение мерки в hash не входят. Для применяемых модельных операций контракт `1.1.0` включает тип, расположение, размеры и версию модуля. Составные детали и слои поднимают контракт до `1.2.0`, покрытие фасона — до `1.3.0`, топологические операции — до `1.4.0`. Запросы без новых модулей сохраняют исторический контракт `1.0.0` и прежние hash. Изменение любой мерки, прибавки, конструктивного параметра, свойства ткани или версии методики меняет hash. JSON канонизируется с сортировкой ключей; `NaN` и бесконечности запрещены.
...
| Persistence | Версионированные проекты, результаты, профили мерок и append-only физические записи | Копии конкретной ревизии и вычисленный gate генерации | Неявных миграций при чтении и ручного флага production |
...
[openapi.v1.yaml](../schemas/openapi.v1.yaml) фиксирует 21 операцию, включая журнал физической приёмки, единый печатный SVG и A4 PDF. `PUT` проекта и профиля использует номер ревизии через `If-Match`, чтобы одна вкладка не затёрла изменения другой. Физические записи добавляются отдельно и не редактируют историческую генерацию. Список профилей намеренно не содержит сами мерки. SQLite работает в WAL-режиме, а документы проходят JSON Schema и смысловую проверку до записи.
