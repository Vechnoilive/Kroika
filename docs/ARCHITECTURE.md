...
Версия контрактов: **1.0.0** · Этап: **5/15** · Стиль: модульный монолит.
...
[openapi.v1.yaml](../schemas/openapi.v1.yaml) фиксирует 18 операций. К API этапа 4 добавлены каталог, проверка мерок и создание/список/чтение/обновление профилей. `PUT` проекта и профиля использует номер ревизии через `If-Match`, чтобы одна вкладка не затёрла изменения другой. Список профилей намеренно не содержит сами мерки. SQLite работает в WAL-режиме, а документы проходят JSON Schema и смысловую проверку до записи.
