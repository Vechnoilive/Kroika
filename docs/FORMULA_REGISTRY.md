...
## M18: модельные преобразования

Все длины — mm. Формулы проверяются по
[эталону этапа 18](../references/stage18/modeling-controls.json). Первые шесть
операций компилируются в геометрию и записываются в `PatternData.modeling_operations`;
последние три пока являются проверенными математическими заготовками и остаются
fail-closed в интерфейсе до реализации сопрягаемых швов.

| ID | Операция | Формула / инвариант | Статус геометрии |
| --- | --- | --- | --- |
| M18-P01 | Центральная складка | `A = k × depth × count`, где `k=2` для односторонней и `k=4` для бантовой/встречной | Реализована |
| M18-G01 | Сборка | `L_cut = L_finished + A_gather`; добавка исключается из эффективной длины соединения | Реализована |
| M18-F01 | Полукруговой волан | `r = L_join / π`, `R = r + depth`; длина внутренней дуги равна срезу низа | Реализована |
| M18-B01 | Прямой пояс | Высота прямоугольной детали равна подтверждённой готовой ширине | Реализована |
| M18-B02 | Отдельный ремень | Прямоугольник `finished_length × finished_width` | Реализована |
| M18-H01 | Расширение/заужение | `W_hem = W_base + delta`, `−100 ≤ delta ≤ 250`, итоговая полуширина ≥ 30 | Реализована в базовом контуре и аудируется |
| M18-Y01 | Горизонтальная кокетка | `L_yoke = depth`, `L_main = L_total − depth`; обе части положительны | Только формула, fail-closed |
| M18-N01 | Равные панели | `x_i = i × W / n`, `i=1…n−1`; сумма ширин равна `W` | Только формула, fail-closed |
| M18-D01 | Перенос вытачки | `intake_source = intake_transferred + intake_retained` | Только формула, fail-closed |

## M19: составные детали и слои

Все длины — mm. Ограниченный каталог проверяется по
[эталону этапа 19](../references/stage19/composite-controls.json) и записывает каждую
применённую операцию в `PatternData.composite_operations`.

| ID | Операция | Формула / инвариант | Статус геометрии |
| --- | --- | --- | --- |
| M19-C01 | Прямая манжета | `L_cuff_join = L_sleeve_hem`; подтверждённая ширина 25–120 | Реализована для длинного рукава блузки/рубашки |
| M19-C02 | Стойка воротника | `L_stand = L_front_neck + L_back_neck`; высота 20–80 | Реализована для bounded-каталога без базового воротника |
| M19-P01 | Парный накладной карман | Прямоугольник `width × depth`, 80–220 × 80–260; путь размещения внутри переда | Реализована для юбочной части |
| M19-L01 | Юбочная подкладка | Точные копии шовных контуров переда/спинки; бок и талия равны исходным соединениям | Реализована |
| M19-L02 | Верхний юбочный слой | Точные копии полотнищ и реализованных воланов; бок и талия равны исходным соединениям | Реализована |
